...
import_base() { # Import a new repository with fallback to a base branch, pass slug and base branch as argument
  ...
  if [ $(git log --oneline -- .github/workflows | head -n 1 | wc -l) = 0 ]; then
    ...
    FILTER_BRANCH_SQUELCH_WARNING=1 git filter-branch --subdirectory-filter .github/workflows --prune-empty 2>&1 | sed -r 's/Rewrite [^)]+[)]//g'
...
merge_into_remote() { # Merge our workflow into the remote repository. Makes worktree unusable. Takes the slug as argument
  ...
  FILTER_BRANCH_SQUELCH_WARNING=1 git filter-branch --env-filter 'export GIT_COMMITTER_DATE="$GIT_AUTHOR_DATE"' --tree-filter 'rm -rf .github/ && mkdir -p .github/workflows/ && mv * .github/workflows/ || true' --prune-empty -f 2>&1 | sed -r 's/Rewrite [^)]+[)]//g'
  cd ../../..
  ...
  if [ $(git log --oneline -- .github/workflows | head -n 1 | wc -l) = 0 ]; then
    ...
    FILTER_BRANCH_SQUELCH_WARNING=1 git filter-branch --subdirectory-filter .github/workflows --prune-empty -f 2>&1 | sed -r 's/Rewrite [^)]+[)]//g'
    FILTER_BRANCH_SQUELCH_WARNING=1 git filter-branch --tree-filter 'rm -rf .github/ && mkdir -p .github/workflows/ && mv * .github/workflows/ || true' --prune-empty -f 2>&1 | sed -r 's/Rewrite [^)]+[)]//g'
